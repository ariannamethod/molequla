...
- The cafeteria: reading is per organism, not a broadcast. A fragment is eaten by the one organism its file name hashes to, and by any other whose own co-occurrence field says the fragment resonates or that it is news. Both bars are quantiles of that organism's own recent coverage: it keeps a ring of its last `experience_coverage_window` measured coverages, persisted beside `dna_cursor.json`, and admits what sits at or above the `experience_resonance_quantile` quantile of it as resonance and at or below the `experience_novelty_quantile` quantile as news. The middle half is declined and the cursor steps past it. No absolute number is compared against: coverage grows with the corpus, so a fixed bar stops cutting anything once an organism's reservoir has grown — the same checkpoint that declined 8 of 9 fragments against a seed corpus declined 0 of 12 against its grown one. Until the ring holds `experience_coverage_warm` samples there is no distribution to quantile, so only the owner rule runs and the rest is declined as `warming`. Nothing is coordinated between the processes — every half is computed from the file name, the bytes, and the reader's own field and history — and the hash owner is what makes every fragment reach somebody, warming included. The element corpus stays a birth condition rather than a profession. Every read pass that judged anything says what it decided in one line — `[cafeteria] <element> admitted=N (owner=a resonance=b novelty=c unmeasured=d) declined=M (band=x warming=y) measured=K`, aggregates only, silent when the routing is off — and `phone1/daily.sh` sums those per organism since its last start into the daily file
...
- Growth budget (phone-1, 2026-09-13): ontogenesis is a memory event, not only an architecture one, so a stage step answers to the same kind of gate as a divide. It needs `MemAvailable` ≥ `CFG.GrowthMinFreeMB` (default 256 MB, 0 disables) + `CFG.GrowthPeakFactorPct` percent of the organism's own peak RSS (default 300, measured from a stage step that multiplied VmHWM by 3.3–3.9×); a closed gate defers and the next tick tries again. Across the colony one `growth_lock` row in mesh.db admits a single grower at a time and is held through the warmup behind it (`CFG.CoordinateGrowth`, default on). Checkpoints are streamed matrix by matrix in both directions instead of being built in memory first — on a 106 MB stage-4 checkpoint that is 337 MB less transient on the way out and a load peak of +152 MB against +349 MB, of which 74 MB is the model itself — every process writes `CFG.OomScoreAdj` (default 300) to its own `oom_score_adj` so the phone's low-memory killer reaches for an organism before the terminal, and a SIGTERM in evolution mode stops the step loop at the next step and writes the checkpoint on the way out. The signal is armed before the bootstrap climb, so a first launch interrupted mid-warmup saves what it trained (`[init] checkpoint saved on signal`) instead of dying with an empty checkpoint. Above both gates sit two declared ceilings, settable per launch and printed as the startup `[caps]` line: `--max-organisms N` (`CFG.MaxOrganisms`, default 16, phone-1 passes 4) and `--max-growth-stage N` (`CFG.MaxGrowthStage`, default the last stage, so unset changes nothing).
- Checkpoint, two files (phone-1, 2026-09-16): an organism writes `molequla_ckpt.json` and, beside it, `molequla_ckpt.gguf` — the same weights as F32 tensors with a directory in front, so they can be mapped instead of parsed. The JSON stays the interchange format between the cores and stays authoritative: it is renamed into place first, and a GGUF that cannot be written is removed rather than left stale beside it. The binary carries every `model.Base` matrix and delta adapter, the embedding snapshot, the shape and growth clock, the tokenizer, and `ActiveAlpha` as exact text; it does not carry Chuck's moment slots, which live in the notorch tape and have never survived a restart (`molequla.chuck_moments` says so in the file). On load the binary is preferred when it is at least as new as the JSON and its identity hash — the whole tokenizer plus the three dimensions that are the growth stage — matches the one read off the front of the JSON; a truncated, stale or foreign sibling falls back to the JSON with one line. Measured on the live organisms of 2026-09-16: 134 095 293 B of JSON against 25 815 360 B of GGUF at stage 4 and 256 665 647 B against 48 614 016 B at stage 5, a save of 2 025 ms against 62 ms, a load of 4 136 ms and +179 MB of high-water against 146 ms and +130 MB, and the same avg loss over 32 steps from either file. Step 2 of `docs/resonator_design.md`.
...
[cafeteria] earth admitted=2 (owner=1 resonance=1 novelty=0 unmeasured=0) declined=3 (band=3 warming=0) measured=4
...
| per organism | stage, params, entropy, `global_step`, the overlay fade of its last generation, and its own peak RSS in MB |
...
Not computed, and why: field coherence and organism resonance need a gamma vector per organism, and no core writes one — the old mycelium read columns that did not exist and reported a constant. Each organism's `global_step` (age in training steps) is in the mesh since 2026-09-13, and beside it since 2026-09-15 its voice — `gen_mag`, the mean |logit| of the raw transformer at the first step of its last generation, and `overlay_fade`, how far the corpus overlay has faded out of it (1 = gone) — and its `peak_rss_mb`, the organism's own `VmHWM` in MB, the number the growth byte gate already charges for and the first figure in the mesh that lets a decision about memory read the colony instead of one process. The witness prints the fade and the peak per organism, `…/f1.00/p755`.
...
- **Training lock**: the colony's burst admission gate (`CFG.SerialBursts`, default on since 2026-09-15; `--no-serial-bursts` restores four concurrent tapes, which is right on a GPU host and wrong on a phone). One `training_lock` row in mesh.db admits one training phase at a time — warmup or micro-burst — taken and checked in a single SQL statement so two organisms cannot both believe they hold it. Behind it a `training_queue` row per waiter carries when it asked and how badly: an organism that has just grown and stands at stage N+1 untrained goes to the head, everyone else in order of longest wait. The holder re-stamps its row three times inside `CFG.TrainingLockTTLSeconds` (default 30), so the TTL bounds a holder that died rather than one that is slow, and a waiter re-asks every `CFG.TrainingTurnPollSeconds` (default 0.5). **Only the burst waits.** The tick body — DNA read and written, the ontogenesis clock, generation — runs first and is never skipped; the 2026-06-03 wall was the same lock's `continue` skipping all of it and freezing three of four organisms for twelve minutes. Each `[notorch] burst complete` line carries `start=` and `end=` in UTC milliseconds, so "no two training phases overlapped" is a fact readable from the stdout files
...
molequla.go              7509 lines   Go organism — lifecycle, ecology, autograd (inference + loss), generation, coherence layer, GPU dispatch, graze, cascade governor, growth budget, streamed checkpoint both ways, the GGUF sibling preferred on load, thread cap
...
cgo_gguf.go              319 lines    CGO bridge to notorch's GGUF reader and writer (gguf.h)
checkpoint_gguf.go       579 lines    the checkpoint as F32 tensors: writer, mapped loader, identity and freshness
...
experience_routing.go    620 lines    The cafeteria (§12/§14): who eats which fragment against quantiles of its own coverage ring, what was just eaten, what may be padded into DNA, and the §13 eligibility gate read from fade and logit magnitude
...
# Tests (228 in package main, 2026-09-15)
...
checkpoint_gguf_test.go  659 lines    GGUF round trip, loss parity against the JSON resume, truncated/foreign/stale siblings refused, no throwaway model, a trained weight is already float32 (8, 1 measurement skipped unless asked)
...
witness_test.go          677 lines    the witness reads what Go writes, schema errors surface, deltas across ticks, harmonics vs DFT, never writes back, the voice and the peak RSS survive a fresh mesh and a migration, four live organisms show four non-zero peaks (10)
training_turn_test.go    358 lines    burst admission: one training phase at a time, the grown organism at the head of the queue, longest wait next, a dead holder freed by the TTL, a live one kept by the refresher, a solo organism never blocked, and four organisms taking turns with zero overlapping phases against thirty without the gate (9)
